...
def validate_constellation_file(file_path, filename):
    ...
    if "sensor_type" in data and data["sensor_type"] not in valid_sensors:
        warnings.append(
            f"Unusual sensor type: {data['sensor_type']} (expected: {valid_sensors})"
        )
    ...
    if "data_access" in data and data["data_access"] not in valid_access:
        warnings.append(
            f"Unusual data access: {data['data_access']} (expected: {valid_access})"
        )
    ...
    if "url" in data:
        ...
        if not isinstance(url, str) or not url.startswith(("http://", "https://")):
            warnings.append(
                f"Invalid URL format: {url} (should start with http:// or https://)"
            )
        if len(url) > 500:
